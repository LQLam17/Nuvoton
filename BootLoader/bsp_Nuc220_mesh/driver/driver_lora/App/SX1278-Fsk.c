...
uint32_t SX1276FskProcess(void)
{
    ...
    case RF_STATE_RX_INIT:
        ...
        Preamble2SyncTimer = RxTimeoutTimer = Timer1_GetTickMs();
    ...
    case RF_STATE_RX_SYNC:
        ...
        if ((DIO4 == 1) && (PreambleDetected == false)) // Preamble
        {
            ...
            Preamble2SyncTimer = Timer1_GetTickMs();
        }
        ...
        if ((SyncWordDetected == false) && (PreambleDetected == true) && ((Timer1_GetTickMs() - Preamble2SyncTimer) > Preamble2SyncTimeout))
        {
        ...
        if ((SyncWordDetected == false) && (PreambleDetected == false) && (PacketDetected == false) && ((Timer1_GetTickMs() - RxTimeoutTimer) > PacketTimeout))
        {
            // Timer_ResetTickMs();
            Timer1_ResetTickMs();
    ...
    case RF_STATE_RX_RUNNING:
        ...
        if (DIO0 == 1) // PayloadReady/CrcOk
        {
            ...
            RxTimeoutTimer = Timer1_GetTickMs();
            if ((RxPacketSize == 0) && (RxBytesRead == 0)) // Read received packet size
            {
                if ((SX1276FSk->RegPacketConfig1 & RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE) == RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE)
                {
        ...
        if ((PacketDetected == false) && ((Timer1_GetTickMs() - RxTimeoutTimer) > PacketTimeout))
        {
            RFState = RF_STATE_RX_TIMEOUT;
        }
    ...
    case RF_STATE_TX_READY_WAIT:
        if (DIO3 == 1) // TxReady
        {
            ...
            TxTimeoutTimer = Timer1_GetTickMs();
    ...
    case RF_STATE_TX_RUNNING:
        ...
        if ((Timer1_GetTickMs() - TxTimeoutTimer) > 1000)
