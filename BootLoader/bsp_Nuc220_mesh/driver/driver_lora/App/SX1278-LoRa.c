...
tLoRaSettings LoRaSettings =
    {
        ...
        300, // RxPacketTimeout (the same as TxPacketTimeout)
        64,  // PayloadLength
        8,   // PreambleLength
};
...
unsigned long int SX1276LoRaProcess(void)
{
    ...
    case RFLR_STATE_RX_RUNNING:
        ...
        if (LoRaSettings.RxSingleOn == TRUE)
        {
            ...
            if (Timer1_GetTickMs() - RxTimeoutTimer > PacketTimeout)
            {
                // Timer_ResetTickMs();
                Timer1_ResetTickMs();
...
uint8_t SX1276CheckLoRaConfigError(void)
{
    ...
    if (CheckFreq != LoRaSettings.RFFrequency)
        result = 1;
    if (CheckPower != LoRaSettings.Power)
        result = 2;
    if (CheckBw != LoRaSettings.SignalBw)
        result = 3;
    if (CheckSF != LoRaSettings.SpreadingFactor)
        result = 4;
    if (CheckCodingrate != LoRaSettings.ErrorCoding)
        result = 5;
    if (CheckCRCon != LoRaSettings.CrcOn)
        result = 6;
    if (CheckPayloadlength != LoRaSettings.PayloadLength)
        result = 7;
    else
        result = 0;
    return result;
